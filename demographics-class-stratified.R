...
# Date: 8/19/2026
# Description: Create demographics table, stratified by latent class membership
...
demo_modified <- demo %>%
  mutate(educcat_mom_mod = case_when(educcat_mom == '>=High School Graduate' ~ '>= High School',
...
# merge demographics data with assigned class memberships
classified <- demo_modified %>%
  ...
  # join demographics data to participant class membership data
  left_join(., 
             classes,
             by = 'newid') %>%
...
# create table -----------------------------------------------------------------
# characteristic variables to include in table
vars <- c('momdl_age2', 
          'educcat_mom_mod',
          'marcat_pg',
          'smoke_yn',
...
# table stratified on asthma trajectories
tbl_summary(
  classified,
  include = vars,
  by = class_label_lca,
